...
execute:
	...
	li t0, 0xFFFFFFFF
	sw t0, 8(sp)          # Store in all 8 words
	sw t0, 12(sp)
	sw t0, 16(sp)
	sw t0, 20(sp)
	sw t0, 24(sp)
	...
	sw t0, 40(sp)         # Alternate the pattern
	sw t1, 44(sp)
	sw t0, 48(sp)
	sw t1, 52(sp)
	sw t0, 56(sp)
	...
	addi a0, sp, 8        # First number
	addi a1, sp, 40       # Second number  
	addi a2, sp, 8        # Result (overwrite first)
	...
	assert.Equal(t, "Execution output: [238, 205, 171, 144, 120, 86, 52, 18, 239, 205, 171, 144, 120, 86, 52, 18, 239, 205, 171, 144, 120, 86, 52, 18, 239, 205, 171, 144, 120, 86, 52, 18]\n", output)
}
